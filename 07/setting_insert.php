...
//３．データ登録SQL作成
$stmt = $pdo->prepare("INSERT INTO interviewee_info(id, interviewee_name, interviewee_name_kana, interview_date, birthday, devision_name, position_name, position_title,
indate )VALUES(NULL, :interviewee_name, :interviewee_name_kana, :interview_date, :birthday, :devision_name, :position_name, :position_title, sysdate())");
...
$stmt->bindValue(':interview_date', $interview_date, PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)
...
if($status==false){
  ...
  //５．index.phpへリダイレクト
  header("Location: interviewee_setting.php");//location: のあとに必ずスペースが入る
  exit;

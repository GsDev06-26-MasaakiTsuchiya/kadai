...
if($status==false){
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    if($tomorrow < $interview_reserve_date_and_time_array[0]){
      ...
      //そうでなければ選択不可
      $view .='<label><input type="radio" name="interview_reserve_time" id="'.h($interview_reserve_time_remove_second).'" value="'.h($result["interview_reserve_time"]).'">'.h($interview_reserve_time_remove_second).'</label>';
    }else{
      $view .='<label class="unselectable"><input type="radio" name="interview_reserve_time" id="'.h($interview_reserve_time_remove_second).'" value="'.h($result["interview_reserve_time"]).'" disabled="disabled">'.h($interview_reserve_time_remove_second).'</label>';
    }
    $view .='</div>';
  }
}
        ...
        <form class="form-horizontal" action="interview_date_time_insert.php" method="post">
          ...
          <div class="form-group text-center">
            <div class="text-center submit_area">
              <a class="btn btn-default" href="interview_date_time_select02.php">戻る</a>
              &emsp;
